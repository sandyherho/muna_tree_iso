...
# Read data
df = pd.read_csv("../data/processed_data/muna_high_pass_stack.csv").set_index("year")
...
plt.xlabel(r'$\delta^{18}{O_c}$ anomaly [‰, VSMOW]', fontsize=16)
plt.legend()
plt.tight_layout()  # Adjust the layout for a tighter plot

# Save fig4a with labels visible
plt.savefig('../figs/fig4a.png', dpi=700, bbox_inches='tight')

# Calculate and print KS test for multiple time periods
pre_periods = [
    df.loc[f"{1695 + i * 25}":f"{1719 + i * 25}"]["median"].dropna().to_numpy()
    for i in range(11)
]
post = df.loc["1970":]["median"].dropna().to_numpy()
...
ax.set_xlabel(r'$\delta^{18}{O_c}$ anomaly [‰, VSMOW]', fontsize=16)
...
plt.tight_layout()
plt.savefig('../figs/fig4b.png', dpi=700, bbox_inches='tight')
